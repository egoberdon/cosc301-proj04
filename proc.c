...
growproc(int n)
{
  ...
  proc->sz = sz;
  if (proc->thread == 1){ //only for threads
    struct proc *p;
    int parent_pid = proc->parent->pid;
    acquire(&ptable.lock);
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      if ((p->pid == parent_pid) || ((p->parent->pid == parent_pid) && (p->thread == 1))){ //p is parent or sibling thread
        p->sz = sz;
      }
    }
    release(&ptable.lock);
...
join(int pid){
  ...
  if (pid != -1){ //waiting for one child
    acquire(&ptable.lock);
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      if((p->pid == pid) && (p->state == ZOMBIE) && (p->parent == proc)){ //Found process
        kfree(p->kstack);
        p->kstack = 0;
        p->state = UNUSED;
        p->pid = 0;
